            ...
            if SOC > self.bt.SOC_min:
                ...
                if abs(energy) <= max_discharge:
                    ...
                    if SOC > self.ht.SOC_Min():
                        ...
                        self.GridToEnergy = abs(energy) - stoToenergy
                        self.energyToStorage = 0
                            ...
                            if SOC > self.LIMIT_SUNNY:
                                if self.dis_enable_ht == True:
                                    ...
                                    if energy <= max_discharge:
                                        ...
                                        self.energyToStorage = 0
                                        self.GridToEnergy = energy - max_discharge - energy_bt
                                        self.storageToEnergy = max_discharge+energy_bt
                                else:
                                    self.GridToEnergy = energy
                                    self.energyToStorage = 0
...
if __name__ == '__main__':
    ...
    ems = HybridESS_OLDS(bt=bt, ht=ht, el_power=230, fc_power=75,t_start=0,t_end=1000,LIMIT_SUNNY=0.20,LIMIT_CLOUDY=0.4)
    ems.initializa()
    ele_cost = 0
    soc_ = []
    gridTopower = 0
