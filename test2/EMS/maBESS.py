...
def energy_management_olds(project_lifetime:int,life_time:int,bt:np.array,pv_output:np.array,pd_load,t_start,t_end,limit_cloudy,
                           limit_sunny,):
    res_output = pv_output


    ems  = BEMS_OLDS(bt=bt,t_start=t_start,t_end=t_end,LIMIT_SUNNY=limit_sunny,LIMIT_CLOUDY=limit_cloudy)
...
if __name__ == '__main__':
    ...
    gridTopower, stoTopower, energyTosto, ele_cost, ele_all = energy_management_olds(project_lifetime=project_lifetime,
                                                                                life_time=life_time, bt=bt,
                                                                                pv_output=pv_output, pd_load=pd_load,t_start=8760,t_end=8763,
